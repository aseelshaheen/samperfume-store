...
import {
  ShoppingBag, Trash2, Plus, Minus, Loader2,
  MapPin, Phone, Tag, ArrowLeft, Package, User,
  CheckCircle, Edit3, Save, X, StickyNote, ChevronDown,
} from "lucide-react";
...
export function guestCartAdd(item) {
  ...
  const idx  = cart.findIndex(
    (i) => i.perfumeId === item.perfumeId && i.section === item.section && i.size === item.size
  );
  if (idx >= 0) {
...
export function guestCartGet() {
  try { return JSON.parse(sessionStorage.getItem(GUEST_CART_KEY) ?? "[]"); }
  catch { return []; }
}
...
export default function Cart() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (isLoggedIn) {
        ...
        const uniqueSlugs = [...new Set(raw.map((i) => i.slug).filter(Boolean))];
        if (uniqueSlugs.length > 0) {
          try {
            ...
            const perfumeMap = {};
            results.forEach((p) => {
              if (p) { perfumeMap[p.slug] = p; perfumeMap[p._id] = p; }
            });
            setCart(raw.map((item) => ({
              ...item,
              perfume: perfumeMap[item.slug] ?? perfumeMap[item.perfumeId] ?? null,
            })));
          } catch { setCart(raw); }
        } else {
          setCart(raw);
  ...
  const activeRegion  = REGIONS.find((r) => r.value === (isLoggedIn ? region : guestRegion));
  const shippingFee   = activeRegion?.shipping ?? null;
  const PROMO_DISC    = 0.20;
  const itemsPrice    = cart.reduce((sum, i) => sum + getItemPrice(i) * i.quantity, 0);
  const discountAmt   = promoApplied ? Math.round(itemsPrice * PROMO_DISC) : 0;
  const totalPrice    = shippingFee != null ? itemsPrice + shippingFee - discountAmt : null;
  ...
  const updateQtyGuest = (gid, delta) => {
    const updated = cart.map((i) =>
      i._guestId === gid ? { ...i, quantity: Math.max(1, i.quantity + delta) } : i
    );
    setCart(updated); guestCartSave(updated);
  };
  ...
  const updateQtyAuth = async (itemId, qty) => {
    if (qty < 1) { removeAuth(itemId); return; }
    try {
  ...
  const removeAuth = async (itemId) => {
    try {
      await fetch(`${API}/users/cart/${itemId}`, { method: "DELETE", headers: authHeaders() });
  ...
  /* ── Promo ── */
  const applyPromo = () => {
    if (promoCode.trim().toUpperCase() === "SAM20") { setPromoApplied(true); setError(""); }
    else setError("كود الخصم غير صحيح");
  };

  /* ── Place guest order ── */
  const placeOrderGuest = async () => {
    setError("");
    if (!guestName.trim())  { setError("يرجى إدخال الاسم الكامل"); return; }
    if (!guestPhone.trim()) { setError("يرجى إدخال رقم الهاتف"); return; }
    if (!guestRegion)       { setError("يرجى اختيار المنطقة"); return; }
    if (!guestCity.trim())  { setError("يرجى إدخال المدينة / البلدة"); return; }
    setPlacing(true);
    try {
      const res = await fetch(`${API}/orders/guest`, {
        ...
        body: JSON.stringify({
          ...
          items: cart.map((item) => ({
            ...
            image:     item.perfume?.images?.find((i) => i.isMain)?.url ?? item.perfume?.images?.[0]?.url ?? null,
            section:   item.section,
            size:      item.section === "taqseem" ? item.size : null,
            quantity:  item.quantity,
            price:     getItemPrice(item),
          })),
          itemsPrice, shippingPrice: shippingFee,
          discount: discountAmt, totalPrice,
  ...
  const placeOrderAuth = async () => {
    ...
    if (!region)       { setError("يرجى اختيار المنطقة"); return; }
    ...
    const items = cart.map((item) => ({
      ...
      image:    item.perfume.images?.find((i) => i.isMain)?.url ?? item.perfume.images?.[0]?.url,
      section:  item.section,
  ...
  if (orderDone) return (
    <>
      ...
      `}</style>
      <div style={{ minHeight:"80vh", display:"flex", alignItems:"center", justifyContent:"center", padding:"2rem", textAlign:"center", direction:"rtl", flexDirection:"column", gap:"1.2rem" }}>
        <div style={{ width:80, height:80, background:"#f0fdf4", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", animation:"pop 0.5s ease both" }}>
          <CheckCircle size={40} color="#2e7d5a" strokeWidth={1.5} />
        </div>
        <h2 style={{ fontFamily:"Playfair Display,serif", fontSize:"1.8rem", color:"#1a1a1a", animation:"rise 0.5s 0.15s ease both", opacity:0 }}>تم تقديم طلبك!</h2>
        <p style={{ color:"#888", fontSize:"0.95rem", maxWidth:340, lineHeight:1.7, animation:"rise 0.5s 0.25s ease both", opacity:0 }}>سنتواصل معك قريباً لتأكيد الطلب وتحديد موعد التوصيل.</p>
        <div style={{ background:"#faf8f6", border:"1px solid #e8e2dc", borderRadius:8, padding:"1rem 2rem", fontSize:"0.88rem", color:"#555", animation:"rise 0.5s 0.35s ease both", opacity:0 }}>
          رقم الطلب: <strong style={{ color:"#452829", fontFamily:"Playfair Display,serif" }}>SP-{orderDone._id?.slice(-5).toUpperCase()}</strong>
        </div>
        <div style={{ display:"flex", gap:"0.8rem", flexWrap:"wrap", justifyContent:"center", animation:"rise 0.5s 0.45s ease both", opacity:0 }}>
          <button onClick={() => navigate("/")} style={{ background:"#452829", color:"white", border:"none", padding:"0.75rem 1.8rem", borderRadius:5, fontFamily:"Tajawal,sans-serif", fontWeight:700, cursor:"pointer" }}>الرئيسية</button>
          <button onClick={() => navigate("/shop")} style={{ background:"white", color:"#452829", border:"1.5px solid #e8e2dc", padding:"0.75rem 1.8rem", borderRadius:5, fontFamily:"Tajawal,sans-serif", fontWeight:700, cursor:"pointer" }}>مواصلة التسوق</button>
        </div>
      </div>
    </>
  );
...
@media(max-width:480px){
  ...
  .divider{margin:0.65rem 0;}
      ...
      {loading ? (
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", padding:"4rem", color:"#452829" }}>
        ...
        <div className="cart-empty">
          ...
          <button onClick={() => navigate("/shop")} style={{ background:"#452829", color:"white", border:"none", padding:"0.72rem 1.6rem", borderRadius:5, fontFamily:"Tajawal,sans-serif", fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", gap:"0.4rem", marginTop:"0.4rem" }}>
        ...
        <div className="cart-layout">
          ...
          <div>
            {cart.map((item, idx) => {
              ...
              const key   = isLoggedIn ? (item._id ?? idx) : (item._guestId ?? idx);

              return (
                <div key={String(key)} className="cart-item">
                  <div className="ci-img-wrap">
                    {img
                      ? <img src={img} alt={p?.name ?? item.name} className="ci-img" />
                      : <div className="ci-img-ph"><Package size={26} /></div>}
                  </div>
                  <div className="ci-body">
                    <span className="ci-brand">{p?.brand ?? item.brand}</span>
                    <span className="ci-name">{p?.name ?? item.name}</span>
                    <span className="ci-section">
                      {item.section === "full" ? "قارورة كاملة" : `تقسيمة · ${item.size ?? ""}مل`}
                    </span>
                  </div>
                  <div className="ci-right">
                    <span className="ci-price">{price > 0 ? `₪${Math.round(price * item.quantity)}` : "—"}</span>
                    <div className="ci-qty">
                      <button className="ci-qty-btn"
                        onClick={() => isLoggedIn ? updateQtyAuth(item._id, item.quantity - 1) : updateQtyGuest(item._guestId, -1)}>
                        <Minus size={12} />
                      </button>
                    ...
                    <button className="ci-remove"
                      onClick={() => isLoggedIn ? removeAuth(item._id) : removeGuest(item._guestId)}>
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
          ...
          <div className="cart-summary">
            ...
            {!isLoggedIn && (
              <>
                <div className="guest-notice">🛍 لا تحتاج لتسجيل دخول — أدخل بياناتك أدناه</div>

                <div className="section-label"><User size={12} /> بياناتك</div>
                <div className="cs-field" style={{ marginTop:"0.4rem" }}>
                  <input className="cs-input" placeholder="الاسم الكامل *" value={guestName} onChange={(e) => setGuestName(e.target.value)} />
                </div>
                <div className="cs-field">
                  <input className="cs-input" placeholder="رقم الهاتف *" value={guestPhone} onChange={(e) => setGuestPhone(e.target.value)} dir="ltr" />
                </div>
                <div className="divider" />

                <div className="section-label"><MapPin size={12} /> عنوان التوصيل</div>
                <div className="cs-field" style={{ marginTop:"0.4rem" }}>
                  <select className="cs-select" value={guestRegion} onChange={(e) => setGuestRegion(e.target.value)}>
                    <option value="">اختر المنطقة *</option>
                    {REGIONS.map((r) => (
                      <option key={r.value} value={r.value}>{r.label} — توصيل ₪{r.shipping}</option>
                    ))}
                  </select>
                </div>
                {guestRegion && (
                  <div className={`ship-badge ${guestRegion === "west_bank" ? "sb-west" : guestRegion === "jerusalem" ? "sb-jerusalem" : "sb-inside"}`}>
                    <MapPin size={11} /> {activeRegion?.label} · رسوم التوصيل ₪{activeRegion?.shipping}
                  </div>
                )}
                <div className="cs-field">
                  <input className="cs-input" placeholder="المدينة / البلدة *" value={guestCity} onChange={(e) => setGuestCity(e.target.value)} />
                </div>
                <div className="divider" />

                <div className="section-label"><StickyNote size={12} /> ملاحظات الطلب</div>
                <div className="cs-field" style={{ marginTop:"0.4rem" }}>
                  <textarea className="cs-textarea" placeholder="ملاحظات للتوصيل أو الطلب (اختياري)" value={guestNotes} onChange={(e) => setGuestNotes(e.target.value)} />
                </div>
              </>
            )}
            ...
            {isLoggedIn && (
              <>
                ...
                <div className="section-hd">
                  <div className="section-label"><Phone size={12} /> رقم الهاتف</div>
                  {phone && (
                    <button className={`edit-btn ${editingPhone ? "active" : ""}`}
                      onClick={() => setEditingPhone(!editingPhone)}>
                      {editingPhone ? <><X size={11} /> إلغاء</> : <><Edit3 size={11} /> تعديل</>}
                    </button>
                  )}
                </div>
                ...
                {!editingPhone && phone ? (
                  <div className="data-pill">
                    ...
                    <span className="data-pill-main" dir="ltr">{phone}</span>
                  </div>
                ) : (
                  <div className="cs-field form-block">
                    <input className="cs-input" type="tel" placeholder="05X XXX XXXX *" value={phone}
                ...
                {/* ── Region ── */}
                <div className="section-label" style={{ marginBottom:"0.45rem" }}><MapPin size={12} /> المنطقة</div>
                <div className="cs-field">
                  <select className="cs-select" value={region} onChange={(e) => setRegion(e.target.value)}>
                    <option value="">اختر المنطقة *</option>
                    {REGIONS.map((r) => (
                      <option key={r.value} value={r.value}>{r.label} — توصيل ₪{r.shipping}</option>
                    ))}
                ...
                {region && (
                  <div className={`ship-badge ${region === "west_bank" ? "sb-west" : region === "jerusalem" ? "sb-jerusalem" : "sb-inside"}`}>
                    <MapPin size={11} /> {activeRegion?.label} · رسوم التوصيل ₪{activeRegion?.shipping}
                  </div>
                )}
                ...
                <div className="section-hd">
                  ...
                  {user?.addresses?.length > 0 && (
                    <button className={`edit-btn ${editingAddr ? "active" : ""}`}
                      onClick={() => { setEditingAddr(!editingAddr); if (!editingAddr) setSelectedAddrId(null); }}>
                      {editingAddr ? <><X size={11} /> إلغاء</> : <><Plus size={11} /> عنوان جديد</>}
                    </button>
                  )}
                </div>
                ...
                {user?.addresses?.length > 0 && !editingAddr && (
                  <div className="addr-list">
                    {user.addresses.map((a) => (
                      <div key={a._id} className={`addr-opt ${selectedAddrId === a._id ? "sel" : ""}`}
                        onClick={() => setSelectedAddrId(a._id)}>
                        <input type="radio" className="addr-radio" readOnly checked={selectedAddrId === a._id} />
                        <div>
                          <div className="addr-lbl">{a.label}{a.isDefault ? " · الافتراضي" : ""}</div>
                          <div className="addr-text">{[a.city, a.area, a.street].filter(Boolean).join("، ")}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {(editingAddr || !user?.addresses?.length) && (
                  <div className="form-block">
                    <div className="cs-row-2" style={{ marginBottom:"0.5rem" }}>
                      <div>
                        <span className="cs-small-label">المدينة *</span>
                        <input className="cs-input" placeholder="مثل: نابلس" value={city} onChange={(e) => setCity(e.target.value)} />
                      </div>
                      <div>
                        <span className="cs-small-label">الحي / المنطقة</span>
                        <input className="cs-input" placeholder="اختياري" value={area} onChange={(e) => setArea(e.target.value)} />
                      </div>
                    </div>
                    <div className="cs-field">
                      <span className="cs-small-label">الشارع</span>
                      <input className="cs-input" placeholder="اختياري" value={street} onChange={(e) => setStreet(e.target.value)} />
                    </div>
                    <div className="save-addr-row">
                      <input type="checkbox" id="saveAddr" checked={saveAddr} onChange={(e) => setSaveAddr(e.target.checked)} />
                      <label htmlFor="saveAddr">حفظ هذا العنوان في ملفي الشخصي</label>
                    </div>
                    {saveAddr && (
                      <div className="cs-field form-block">
                        <span className="cs-small-label">اسم العنوان</span>
                        <input className="cs-input" placeholder="مثل: المنزل، العمل..." value={addrLabel} onChange={(e) => setAddrLabel(e.target.value)} />
                      </div>
                    )}
                  </div>
                )}
                ...
                {/* ── Order notes ── */}
                <div className="section-label" style={{ marginBottom:"0.45rem" }}><StickyNote size={12} /> ملاحظات الطلب</div>
                <div className="cs-field">
                  <textarea className="cs-textarea"
            ...
            {/* ── Promo code ── */}
            <div className="section-label" style={{ marginBottom:"0.45rem" }}><Tag size={12} /> كود الخصم</div>
            {promoApplied ? (
              <div className="promo-ok">✓ تم تطبيق خصم 20% — SAM20</div>
            ) : (
              <div className="promo-row">
                <input className="promo-input" placeholder="أدخل الكود"
                  value={promoCode} onChange={(e) => { setPromoCode(e.target.value); setError(""); }} />
            ...
            {/* ── Price breakdown ── */}
            <div className="row"><span>المجموع الفرعي</span><span>₪{Math.round(itemsPrice)}</span></div>
            <div className="row">
              <span>رسوم التوصيل</span>
              <span>{shippingFee != null ? `₪${shippingFee}` : "اختر المنطقة"}</span>
            </div>
            {promoApplied && <div className="row disc"><span>خصم الكود</span><span>-₪{discountAmt}</span></div>}
            <div className="divider" />
            <div className="row total">
              <span>الإجمالي</span>
            <span>{totalPrice != null ? `₪${Math.round(totalPrice)}` : "—"}</span>
            </div>
            ...
            <p className="cs-note">الدفع عند الاستلام · شحن خلال 2–4 أيام عمل</p>
          </div>
