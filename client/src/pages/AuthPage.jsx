...
const API_BASE = "/api";
const ADMIN_EMAIL = "aseelshaheen080@gmail.com";
...
export default function AuthPage({ onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    if (mode === "register" && form.password !== form.confirm) return setError("كلمتا المرور غير متطابقتين");
  ...
  return (
    <>
      <style>{`
        ...
        :root {
          --bob:#452829; --bob-hover:#5c3637; --black:#0e0e0e;
          --surface:#faf8f6; --border:#e5dfd8; --text:#1a1a1a;
          --muted:#888; --error:#c0392b; --success:#2e7d5a; --white:#ffffff;
        }
        body { font-family:'Tajawal',sans-serif; direction:rtl; margin:0; background:var(--white); color:var(--text); }
        .auth-root { min-height:100vh; display:grid; grid-template-columns:1fr 1fr; }
        .auth-visual {
          position:relative; background:#222020; display:flex; flex-direction:column;
          align-items:center; justify-content:flex-start; padding:3rem; overflow:hidden;
        }
        .image-container { position:relative; z-index:2; margin-bottom:2rem; }
        .image-container img { width:100%; max-width:500px; height:auto; object-fit:cover; border-radius:12px; opacity:0.8; filter:saturate(0.9); }
        .auth-visual-content { position:relative; z-index:2; text-align:center; color:white; }
        .visual-tagline { font-size:0.7rem; letter-spacing:0.26em; text-transform:uppercase; color:#d4c4b8; font-weight:600; display:block; }
        .orb { position:absolute; border-radius:50%; filter:blur(80px); opacity:0.18; pointer-events:none; z-index:1; }
        ...
        <div className="auth-form-panel">
          <div className="auth-card">
            <div className="auth-tabs">
              <button className={`tab-btn ${mode === "login" ? "active" : ""}`} onClick={() => switchMode("login")}>تسجيل الدخول</button>
              <button className={`tab-btn ${mode === "register" ? "active" : ""}`} onClick={() => switchMode("register")}>إنشاء حساب</button>
            </div>

            <h2 className="auth-heading">{mode === "login" ? "أهلاً بعودتك" : "انضم إلينا"}</h2>
            <p className="auth-sub">{mode === "login" ? "سجّل دخولك للوصول إلى حسابك وطلباتك" : "أنشئ حسابك وابدأ رحلة العطور الفاخرة"}</p>
            ...
            <div className="field">
              ...
              <input type="email" placeholder="example@email.com" value={form.email} onChange={set("email")} onKeyDown={handleKey} />
            </div>
            <div className="field">
              <label>كلمة المرور</label>
              <div className="input-wrap">
                <input type={showPw ? "text" : "password"} placeholder="••••••••" value={form.password} onChange={set("password")} onKeyDown={handleKey} style={{ paddingLeft: "2.4rem" }} />
            ...
            {mode === "register" && (
              <div className="field">
                ...
                <input type={showPw ? "text" : "password"} placeholder="••••••••" value={form.confirm} onChange={set("confirm")} onKeyDown={handleKey} />
              </div>
            )}
            ...
            <button className="submit-btn" onClick={handleSubmit} disabled={loading}>
              {loading
                ...
                : <>{mode === "login" ? "دخول" : "إنشاء الحساب"}<ArrowLeft size={16} /></>}
            </button>
