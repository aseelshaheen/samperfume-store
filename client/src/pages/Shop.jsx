...
import {
  ...
  ShoppingBag, Eye, Layers, Package,
  ArrowLeftRight, Loader2, Wind
} from "lucide-react";
...
const TYPES    = [{ v: "", l: "الكل" }, { v: "arabic", l: "عربية" }, { v: "western", l: "أجنبية" }];
...
function EmptyState({ section }) {
  return (
    <div className="empty-state">
      <div className="empty-icon"><Wind size={32} strokeWidth={1.2} /></div>
      <h3>لا توجد عطور</h3>
      <p>{section === "full" ? "لم يتم إضافة قوارير كاملة في هذا القسم حتى الآن." : "لم يتم إضافة تقسيمات في هذا القسم حتى الآن."}</p>
...
function PerfumeCard({ p, section }) {
  ...
  const crossLink = p.availability === "both";
  ...
  return (
    <div className="p-card" onClick={goToDetail} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <div className="p-card-img-wrap">
        {mainImg
          ? <img src={mainImg} alt={p.name} className="p-card-img" style={{ transform: hovered ? "scale(1.06)" : "scale(1)" }} />
          : <div className="p-card-img-placeholder"><Package size={36} strokeWidth={1} /></div>}

        {p.isFeatured && <span className="p-tag featured">مميز</span>}
        {p.discount > 0 && <span className="p-tag discount">-{p.discount}%</span>}

        <div className={`p-card-actions ${hovered ? "visible" : ""}`}>
          <button className="p-action" onClick={goToDetail}><Eye size={14} /></button>
          {/* No wishlist button — guest mode */}
          <button className="p-action primary" onClick={goToDetail}><ShoppingBag size={14} /><span>تفاصيل</span></button>
        </div>
      </div>
      ...
      <div className="p-card-body">
        ...
        <div className="p-meta-row">
          <span className={`p-type-badge ${p.perfumeType}`}>{p.perfumeType === "arabic" ? "عربي" : "أجنبي"}</span>
          <span className="p-gender">{p.gender === "male" ? "رجالي" : p.gender === "female" ? "نسائي" : "مشترك"}</span>
        </div>
        {section === "taqseem" && p.taqseem?.sizes?.length > 0 && (
          <div className="p-sizes">
            {p.taqseem.sizes.map(s => <span key={s.ml} className="p-size-pill">{s.ml}مل</span>)}
          </div>
        )}
        <div className="p-price-row">
          <span className="p-price">{finalPrice !== "—" ? `₪${finalPrice}` : "—"}</span>
          {originalPrice && <span className="p-original">₪{originalPrice}</span>}
          {section === "taqseem" && p.taqseem?.sizes?.length > 1 && <span className="p-from">يبدأ من</span>}
        </div>
      </div>
    </div>
  );
...
export default function Shop() {
  ...
  useEffect(() => {
    fetcher(`${API}/perfumes/brands`).then(d => setBrands(d.brands ?? [])).catch(() => {});
  }, []);
  ...
  }, [debouncedSearch, brand, type, gender, family, sort]);
  ...
  const clearFilters = () => { setSearch(""); setBrand(""); setType(""); setGender(""); setFamily(""); setSort("newest"); };
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .p-crosslink:hover{background:rgba(255,255,255,1);}
        ...
        .p-name{font-family:'Playfair Display',serif;font-size:0.95rem;color:var(--dark);font-weight:600;margin-bottom:0.5rem;line-height:1.35;}
  ...
  .shop-toolbar{padding:0.6rem 0.75rem;top:0;}
  .shop-toolbar-inner{gap:0.4rem;}
  .search-wrap{min-width:0;max-width:100%;flex:1;}
  .search-wrap input{font-size:0.8rem;padding:0.45rem 1.8rem 0.45rem 0.6rem;width:100%;}
  .filter-toggle{padding:0.45rem 0.65rem;font-size:0.78rem;}
  .sel{padding:0.45rem 1.6rem 0.45rem 0.6rem;font-size:0.78rem;}
  ...
  .p-name{font-size:0.8rem;margin-bottom:0.25rem;line-height:1.25;}
      ...
      <div className="shop-header">
        <div className="shop-header-inner">
          ...
          <div className="section-tabs">
            <button className={`section-tab ${activeSection === "full" ? "active" : ""}`} onClick={() => setActiveSection("full")}>
              <Package size={16} /> القوارير الكاملة <span className="tab-count">{fullBottles.length}</span>
            </button>
            <button className={`section-tab ${activeSection === "taqseem" ? "active" : ""}`} onClick={() => setActiveSection("taqseem")}>
              <Layers size={16} /> التقسيمات <span className="tab-count">{taqseemBottles.length}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="shop-toolbar">
        <div className="shop-toolbar-inner">
          <div className="search-wrap">
            ...
            <input type="text" placeholder="ابحث عن عطر أو براند..." value={search} onChange={e => setSearch(e.target.value)} />
          ...
          <span className="result-count">{loading ? "..." : `${displayed.length} عطر`}</span>
          {hasActiveFilters && (
            <button className="clear-btn" onClick={clearFilters}><X size={13} /> مسح الفلاتر</button>
          )}
        </div>
      </div>
          ...
          <>
            <div className="section-head">
              <div className="section-head-left">
                <div className="section-icon">{activeSection === "full" ? <Package size={16} /> : <Layers size={16} />}</div>
